  ...
  return (
    <div className="bg-white rounded border border-gray-200/50 p-4 hover:shadow-sm transition-shadow">
      ...
      <div className="grid grid-cols-2 gap-3 text-xs">
        <div>
          <span className="text-gray-400 font-medium uppercase tracking-wide">Ward</span>
          <p className="text-gray-700 font-semibold">
            {admission.ward?.name} / Bed {admission.bed?.bed_number}
          </p>
        </div>
        <div>
          <span className="text-gray-400 font-medium uppercase tracking-wide">Type</span>
          <p className="text-gray-700 font-semibold">{admission.admission_type}</p>
        </div>
        <div>
          <span className="text-gray-400 font-medium uppercase tracking-wide">Diagnosis</span>
          <p className="text-gray-700 truncate">{admission.admission_diagnosis || "-"}</p>
        </div>
        <div>
          <span className="text-gray-400 font-medium uppercase tracking-wide">Acuity</span>
          <p className="text-gray-700 font-semibold">{admission.acuity_level}</p>
        </div>
        <div>
          <span className="text-gray-400 font-medium uppercase tracking-wide">Admitted</span>
          <p className="text-gray-700 font-mono">
            {new Date(admission.admission_date).toLocaleDateString()}
          </p>
        </div>
        <div>
          <span className="text-gray-400 font-medium uppercase tracking-wide">Isolation</span>
      ...
      {admission.length_of_stay_days !== null && (
        <div className="mt-3 pt-3 border-t border-gray-100">
          <span className="text-xs text-gray-400">LOS: </span>
